...
namespace Common {
    struct TCPServer {
        ...
        // Check for new connections/dead connections and update containers that track the sockets
        auto poll() noexcept -> void;

        // Publish outgoing data from the send bufffer and read incoming data from the receive buffer
        ...
        public:
            // Socket on which this server is listening for new connections
            int epoll_fd_ = -1;
            TCPSocket listener_socket_;

            epoll_event events_[1024];

            // Collection of all sockets, sockets for incoming data, for outgoing data and dead connections.
            ...
            // Function wrapper to call back when all data accross all TCPSockets has been read and dispatched this round
            std::function<void()> recev_finished_callback_ = nullptr;
