...
namespace Common {
    ...
    [[nodiscard]] inline auto createSocket(Logger& logger, const SocketCfg& socket_cfg) -> int {
        ...
        for(addrinfo* rp = result; rp; rp = rp->ai_next) {
            ...
            {
                const int rc_connect = connect(fd, rp->ai_addr, rp->ai_addrlen);
                // In non-blocking mode, connect returns 0 on immediate success, or -1 with EINPROGRESS while connecting
                if (rc_connect != 0) {
                    const int saved_errno = errno;
                    ASSERT(saved_errno == EINPROGRESS, "connect() failed, errno:" + std::string(strerror(saved_errno)));
                }
            }
            ...
            if (socket_cfg.is_listening_) // allow re-using the address in the call to bind
            {
                ...
                const int rc_bind = bind(fd, socket_cfg.is_udp_ ? reinterpret_cast<const struct sockaddr*>(&addr) : rp->ai_addr, 
                                         socket_cfg.is_udp_ ? sizeof(addr) : rp->ai_addrlen);
                if (rc_bind != 0) {
                    const int saved_errno = errno;
                    ASSERT(false, "bind() failed, errno:" + std::string(strerror(saved_errno)));
                }
            }
